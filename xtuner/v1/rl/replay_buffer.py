...
class ReplayBuffer:
    ...
    async def refresh_onpolicy_completed_masks(self, *, task_model_steps: dict[str, int]) -> None:
        """Restrict completed samples to tokens generated by the required model step.

        Completed groups containing a sample without any trainable token from the required model step are discarded.
        For aborted groups, only stale completed members are reset so unfinished members can retain their partial
        responses and continue generation. This preserves fixed-size RL groups, avoids all-ignore samples, and keeps
        agentic long-tail partial rollouts reusable.

        Args:
            task_model_steps (dict[str, int]): Required rollout model step for each task.
        """
        async with self._lock:
            updated_records: list[StorageItem] = []
            discarded_uids: list[int] = []
            for task_name, required_model_step in task_model_steps.items():
                query_dsl: QueryDict = {
                    "$and": [
                        {"task_name": task_name},
                        {"status": {"$in": [Status.COMPLETED, Status.ABORTED]}},
                    ]
                }
                records = await self._storage.get(query_dsl)
                for record in records:
                    group_status = get_group_status(record.item)
                    group_is_valid = True
                    for sample in record.item:
                        response_ids = sample.response_ids or []
                        response_model_steps = sample.response_model_steps or []
                        response_mask = sample.response_mask
                        if response_mask is None or len(response_mask) == 0:
                            response_mask = [1] * len(response_ids)

                        if not (
                            len(response_ids) == len(response_model_steps) == len(response_mask)
                        ):
                            group_is_valid = False
                            break

                        current_response_mask = [
                            int(bool(mask) and model_step == required_model_step)
                            for mask, model_step in zip(response_mask, response_model_steps)
                        ]
                        if sample.status != Status.COMPLETED:
                            continue

                        sample.response_mask = current_response_mask
                        if any(sample.response_mask):
                            continue
                        if group_status == Status.ABORTED:
                            reset_rollout_response(sample)
                    ...
                    for sample in record.item:
                        discard_rollout_state(sample)
                    discarded_uids.append(record.uid)
            await self._storage.update(updated_records)
            await self._storage.delete(discarded_uids)
